...
public class RobotTest {
	...
	public void robotTest() throws Exception {
		// Create a new simulator and warehouse from file so that the robot can assess
		// jobs
		...
		// Test to ensure that the acceptJob method is working successfully, robot won't
		// accept job without enough power units
		boolean test = true;
		try {
		...
		// Ensure the robot has enough charge to accept the job, and test the accept
		// method again
		r.charge(8, 20);
		test = true;
		try {
		...
		// Test tick when robot has accepted job and needs to collect item (from two
		// spaces away so after two ticks the robot should have the item).
		r.setPowerUnits(20);
		r.tick(warehouse);
		...
		// check that robot returns the item to the packing station when it has the
		// item.
		r.tick(warehouse);
