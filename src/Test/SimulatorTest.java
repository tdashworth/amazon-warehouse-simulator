...
public class SimulatorTest {
	...
	public void runSimulatorTest() {
		...
		//Run the simulator, then test to ensure that the simulator completed successfully
		try {
			s.run();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void tickSimulatorTest() {
		...
		//Make the simulator tick, and then check to see if the expected happened, repeatedly
		try {
			s.tick();
		} catch (Exception e) {
			e.printStackTrace();
		...
		assertNotNull(((PackingStation) s.getActors().get(0)).getCurrentOrder()); //Packing station
		assertNull(s.getWarehouse().getUnassignedOrder());
		...
		assertEquals("ss0", ((PackingStation) s.getActors().get(0)).getStorageShelvesVisited().get(0).getUID()); //Packing station
		assertNull(s.getWarehouse().getUnassignedOrder());
		}
